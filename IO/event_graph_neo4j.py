import os
from Neo4j.Neo4J_tool import Neo4jGraph

class EventGraph:
    # 关系文件位置
    EVENT_RELATIONS_LIST_FILE_NAME = os.path.join(
        os.path.abspath(os.path.dirname(os.getcwd())), "Data/event_relations.csv")  #合并后的事件关系文件
    ...
    def get_graph_nodes_and_edges(self):
        ...
        for event_triple_set in sorted(event_dict.items(), key=lambda asd: asd[1], reverse=True)[:10000]:
...
if __name__ == "__main__":
    ...
    print("Edges:")
    event_relation = nodes_and_edges[1]
    print(event_relation)

    neo4j_graph = Neo4jGraph()
    neo4j_graph.Node_Relation(nodes_and_edges[1]) #将事件关系组导入neo4j图数据库