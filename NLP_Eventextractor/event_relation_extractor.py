import re
from NLP_Eventextractor.ltp_parser import LtpParser
...
class EventRelationExtractor:
    ...
    def pattern_causality_1(self):
        keywords = [[['因为'], ['从而', '为此', '因而', '致使', '以致于?', '以至于?', '所以', '于是', '故', '故而', '因此'],
                    ...
                    [[], ['从而', '为此', '因而','造成', '致使', '以致于?', '以至于?', '所以', '于是', '故', '故而', '因此'], 'causality'],
                    [[], ['致使', '结果', '牵动', '导向', '导致','带动', '勾起', '引入', '指引', '予以', '产生', '促成', '造成', '引导', '造就', '促使', '酿成',
                          '引发', '渗透', '促进', '引起', '诱导', '引来', '促发', '引致', '诱发', '推进', '诱致', '推动', '招致', '影响下?', '致使',
                          '滋生', '归于', '作用', '使得', '决定', '攸关', '令人', '引出', '浸染', '带来', '挟带', '触发', '关系到', '渗入', '诱惑',
                          '波及', '诱使'], 'causality'],
                    [['为了','推动', '依据', '按照', '因为', '因', '按', '依赖', '凭借', '由于', '既然'], ['，'], 'causality'],
...
if __name__ == '__main__':
    ...
    content6 = "从 2018 年 8 月以来的非洲猪瘟对我国猪肉全产业链影响导致实际产能减少、猪肉价格严重分化、跨省运输停滞，北方生猪养殖企业大面积亏损"
    event_relation_extractor = EventRelationExtractor()
    event_relation_triples = event_relation_extractor.event_relation_extrator_main(content6)
